...
Ptr<SVM> trainSymbolsFull(const Mat_<float> &dataMat, const Mat_<int> &labelMat);
...
int main(int argc, char* argv[]) {
	exampleForTrainAndPredictMNIST();
	...
	trainSVM(trainFeatures,trainLabels);
	*/
...
void trainSymbols()
{
	...
	getAllFormatFiles("symbols/", symbol_paths, "jpg");

	vector<Mat> symbols;
	Mat_<int> slabels;
	slabels = Mat_<int>(symbol_paths.size(), 1);
	cout << symbol_paths.size() << "Symbols Found\n";

		for (int i = 0; i<symbol_paths.size(); i++)
		{
			cout << symbol_paths[i] <<"\n";
			
			cout << "Label Is" << symbol_paths[i].at(9) <<"\n";
			Mat simg = imread(symbol_paths[i]);
			char slbl = symbol_paths[i].at(9);

			Mat img;
			img.create(28,28, CV_8UC1);
			cv::resize(simg, simg, img.size());
			symbols.push_back(simg);
			slabels.at<int>(i, 0) = (int)slbl;

		}
		Mat_<float> symbolFeatures = extractFeatures(symbols);

		Ptr<SVM> symbol = trainSymbolsFull(symbolFeatures, slabels);
		symbol->save("dataset/mysymbols.xml");
...
Ptr<SVM> trainSymbolsFull(const Mat_<float> &dataMat, const Mat_<int> &labelMat) {
	...
	cout << "SVM training of Symbols start." << endl;

	Ptr<SVM> svm = SVM::create();

	svm->setType(SVM::Types::C_SVC);
	svm->setKernel(SVM::KernelTypes::LINEAR);
	svm->setDegree(3);  // for poly
	...
	cout << "SVM Training of Symbols Completed." << endl;
...
void cutDigitsAndPredict(Mat src) {
	...
	Mat canny_output;
	Canny(threshold_output, canny_output, thresh, thresh * 5, 3);
	...
	//Ptr<SVM> svm = Algorithm::load<SVM>("dataset/mysymbols.xml");
...
void exampleForTrainAndPredictMNIST() {
	...
	if (!file1.is_open() || !file2.is_open()) {
		...
		if (!file1.is_open()) {
			...
			cout << "The accuracy of the SVM is " << result << ", on a total of " << predictLabels.size().height << " images." << endl;


			cout << "\nWould you like to see more SVM test cases in detail?" << endl;
			cout << "Press y to see, n to skip." << endl;
			char t;
			while (cin >> t) {
				if (t == 'y') {
					cout << "Total of 20 images." << endl;
					srand(time(0));
...
Ptr<Boost> trainBoost(const Mat_<float> &dataMat, const Mat_<int> &labelMat) {
	...
	cout << "Training the adaboost classifier (may take a few minutes)" << endl;
...
Mat_<float> extractFeatures(const vector<Mat> &digits) {
	...
	cout << "Starting extraction of features." << endl;
...
vector<Mat> readDigits(string filePath) {
	...
	if (!file.is_open()) {
		...
		cout << "Started reading digits." << endl;
	...
	cout << "Finished reading digits." << endl;
...
Mat_<int> readLabels(string filePath) {
	...
	{
		cout << "Started reading labels." << endl;
		int magic_number = 0;
